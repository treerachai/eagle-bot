    ...
    if (suffix) {
      if (index[1] !== undefined && index[0].toLowerCase() === 'create') {
        ...
        r.db('Discord').table('Tags').get(index[1].toLowerCase()).run().then((g) => {
          if (g !== null) {
            msg.reply('This tag already exists.')
          }
        })
        r.db('Discord').table('Tags').insert({
          ...
          if (g.inserted === 1) {
            msg.reply('Tag successfully created.')
          } else if (g.errors > 1) {
        ...
        r.db('Discord').table('Tags').get(index[1].toLowerCase()).run().then((g) => {
          if (g === null) {
            msg.reply('This tag does not exist.')
          } else {
            msg.reply(`The owner of that tag is ${bot.Users.get(g.owner) !== null ? bot.Users.get(g.owner).username : '`Unknown`'}`)
          }
        })
        ...
        r.db('Discord').table('Tags').get(index[1].toLowerCase()).run().then((g) => {
          if (g === null) {
            ...
            if (g.owner !== msg.author.id && Config.permissions.master.indexOf(msg.author.id) === -1) {
              msg.reply('That tag is not yours to edit.')
            } else {
              if (Config.permissions.master.indexOf(msg.author.id) === -1) {
                var re = /(discord(\.gg|app\.com\/invite)\/([\w]{16}|([\w]+-?){3}))/
                if (msg.mentions.length >= 5) {
              ...
              r.db('Discord').table('Tags').get(index[1].toLowerCase()).update({
                ...
                if (g.replaced === 1) {
                  msg.reply('Tag successfully edited.')
                } else if (g.errors > 1) {
                  msg.reply('Something went wrong.')
                }
              })
        ...
        r.db('Discord').table('Tags').get(index[1].toLowerCase()).run().then((g) => {
          if (g === null) {
            ...
            if (g.owner !== msg.author.id && Config.permissions.master.indexOf(msg.author.id) === -1) {
              msg.reply('That tag is not yours to delete.')
            } else {
              r.db('Discord').table('Tags').get(index[1].toLowerCase()).delete().run().then((g) => {
                if (g.deleted === 1) {
                  msg.reply('Tag successfully deleted.')
                } else if (g.errors > 1) {
                  msg.reply('Something went wrong.')
                }
              })
        ...
        r.db('Discord').table('Tags').filter({owner: author.id}).count().run().then((c) => {
          if (c === 0) {
            msg.reply(`${msg.author.id === author.id ? "You don't" : 'This user does not'} have any tags!`)
          } else {
            var tagsArray = []
            r.db('Discord').table('Tags').filter({owner: author.id}).run().then((tags) => {
              ...
              if (tagsArray.join(', ').length > 1950) {
                msg.reply(`The length of ${msg.author.id === author.id ? 'your' : "this user's"} tag list exceeds 2000 characters.`)
                return
              }
              msg.reply(`Found ${c} tags for **${author.username}**:\n${tagsArray.join(', ')}`)
            })
          }
        })
        ...
        r.db('Discord').table('Tags').count().run().then((c) => {
          if (c === 0) {
            msg.reply('No tags found in the database.')
          } else {
      ...
      msg.reply('No arguments entered.')
    }
  }
}

exports.Commands = Commands
exports.Filename = "Tag";
