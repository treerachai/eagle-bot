...
var field = [
  { name: prefix + 'request', value: `Request a song via a youtube or soundcloud link, or any kind of compatible music file.`, inline: false },
  { name: prefix + 'music pause', value: 'Pauses the current song.', inline: false },
  { name: prefix + 'music play', value: 'Resumes the current song.', inline: false },
  { name: prefix + 'volume', value: 'Change the volume of the current song.', inline: false },
  { name: prefix + 'playlist', value: 'List upcoming requested songs.', inline: false },
  { name: prefix + 'shuffle', value: 'Shuffle the music playlist.', inline: false },
  { name: prefix + 'voteskip', value: 'Vote to skip the current song.', inline: false },
  { name: prefix + 'skip', value: 'Force skip the current song.', inline: false },
  { name: prefix + 'leave-voice', value: 'Leaves the voice channel.', inline: false }
]
var embed = {
      ...
      if (VC) {
        VC.join().then((vc) => {
          ...
          require('../datacontrol.js').customize.prefix(msg).then((r) => {
            ...
            var joinMessage = joinmsg.concat(voiceCmds)
            msg.channel.sendMessage(joinMessage.join('\n'))
    ...
    .map((connection) => {
      if (connection.voiceConnection.guild.id === msg.guild.id) {
        ...
        encoder.once('end', () => {
          ...
          if (Config.settings.replay) {
            list[msg.guild.id].link.push(list[msg.guild.id].link.shift())
            list[msg.guild.id].info.push(list[msg.guild.id].info.shift())
            list[msg.guild.id].requester.push(list[msg.guild.id].requester.shift())
          }
          else {
            list[msg.guild.id].link.shift()
            list[msg.guild.id].info.shift()
  ...
  return new Promise(function (resolve, reject) {
    if (list[msg.guild.id].info === undefined) {
      ...
      try {
        ...
        list[msg.guild.id].skips.count = 0
        list[msg.guild.id].skips.users = []
        played[msg.guild.id].info.splice(1)
        played[msg.guild.id].link.splice(1)
        played[msg.guild.id].requester.splice(1)
        played[msg.guild.id].skips.count = 0
        played[msg.guild.id].skips.users = []
